# HATCHLING CONSTRUCTION KIT — MASTER GUIDE
...
- **Game file:** `project/Hatchling Quest 1.html` — the game itself
- **Bridge:** `project/hatchling_data.json` — exported by the kit, loaded by the game at startup
...
`hatchling_data.json`. The game loads that file on boot and merges it over its
hardcoded defaults. If the JSON is missing, the game uses its built-in defaults.

```
construction_kit.html  ──EXPORT──▶  hatchling_data.json  ──fetch on load──▶  Hatchling Quest 1.html
```
...
- Everything auto-saves to `localStorage` (`hatchling_kit_v4`). **SAVE** forces it.
...
> not copy them into `scenesOut`** — they don't reach `hatchling_data.json`
...
## 10. EXPORT — `hatchling_data.json` SCHEMA
...
### How the game merges it (loader in `Hatchling Quest 1.html`)
...
Cutscenes export under `"cutscenes"` in `hatchling_data.json`:
```jsonc
"cutscenes": {
  "opening": {
    "nodes": [
